...
def getInfo(i):
    url = 'http://neimenggu.chinatax.gov.cn/nsfw/sscx/zdaj/'
    driver = getDriver()
    driver.get(url)
    # 纳税人名称
    nsrmc = '通辽市兴达房地产开发有限公司'
    # nsrmc='北京字节跳动科技有限公司'
    driver.find_element_by_xpath('//*[@id="title"]').send_keys(nsrmc)
    time.sleep(1)
    driver.find_element_by_xpath('//*[@id="advsearchform"]/div[10]/input[1]').click()
    time.sleep(1)
    all_h = driver.window_handles
    driver.switch_to.window(all_h[1])
    all_h = []
    result = driver.find_element_by_xpath('//*[@id="jjuuu"]/table/tbody').get_attribute('innerHTML')
    if re.findall(r"没有找到和您的查询相匹配的结果", result):
        print('无符合公布标准的案件信息')
        driver.quit()
        exit(0)
    else:
        # print(re.sub(u"\\<.*?\\>","",result))
        driver.find_element_by_xpath('//*[@id="jjuuu"]/table/tbody/tr['+str(i)+']/td[5]/a').click()
        time.sleep(1)
        all_h = driver.window_handles
        driver.switch_to.window(all_h[3])
        info = driver.find_element_by_xpath('//*[@id="bgname"]/div/div/div[1]/table/tbody').get_attribute('innerHTML')
        # print(info)
...
//*[@id="jjuuu"]/table/tbody/tr[1]/td[5]/a
//*[@id="jjuuu"]/table/tbody/tr[2]/td[5]/a
//*[@id="jjuuu"]/table/tbody/tr[12]/td[5]/a

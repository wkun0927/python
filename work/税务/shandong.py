...
#税务违法   山东信息获取   详细信息里边没内容 所以爬取外部信息
...
def getInfo(i):
    url = 'https://etax.shandong.chinatax.gov.cn/dzswj/zdsswfsxajgbl.jsp'
    driver = getDriver()
    driver.get(url)
    # 纳税人名称
    nsrmc = '聊城桂雷新能源有限公司'
    ...
    time.sleep(1)
    driver.find_element_by_xpath('//*[@id="wtdzkpcxForm"]/table/tbody/tr[5]/td[2]/button').click()
    time.sleep(1)
    driver.switch_to.frame('content')
    result = driver.find_element_by_xpath('//*[@id="form1"]/div[1]/table/tbody/tr[2]/td').get_attribute('innerHTML')
    # print(result)
    if re.findall(r"无符合公布标准的案件信息", result):
        print('无符合公布标准的案件信息')
    else:
        info = driver.find_element_by_xpath('//*[@id="form1"]/div[1]/table/tbody/tr['+str(i)+']').get_attribute('innerHTML')
        info1 = re.sub(u"\\</tr\\>", ",", info)
        info2 = re.sub(u"\\</td\\>", "、", info1)
        info3 = re.sub(u"\\<.*?\\>", "", info2)
        info4 = re.sub(u"\n", "", info3)
        info5 = re.sub(u"\t", "", info4)
        info6 = info5.replace(' ', '').replace('&nbsp;','').replace('、',',').split(',')
        print(info6)
...
'''
//*[@id="form1"]/div[1]/table/tbody/tr[2]
//*[@id="form1"]/div[1]/table/tbody/tr[3]
//*[@id="form1"]/div[1]/table/tbody/tr[11]
'''